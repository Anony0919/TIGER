...
class GNNLayer(torch.nn.Module):
    def __init__(self, params, hidden_dim):
        ...
        self.relation_embed = nn.Embedding(2 * self.n_rel + 1, self.in_dim)
    ...
    def forward(self, query, layer_input, edges, nodes, n_ent, edge_count):
    ...
    def forward2(self, query, layer_input, edges, n_ent,edge_count = None):
        ...
        if self.AGG_FUNC == 'pna':
            message_agg = self.pna_process(message, obj, item_size=n_ent,
                                           edge_count=edge_count.unsqueeze(2), scatter_dim=1)
        elif self.AGG_FUNC == 'mean':
            count_sum = edge_count.unsqueeze(2).clamp(min=1)
            sum = scatter(message, index=obj, dim=1, dim_size=n_ent, reduce="sum")
            message_agg = sum / count_sum
        else:
            message_agg = scatter(message, index=obj_index, dim=1, dim_size=n_ent, reduce=self.AGG_FUNC)
...
class GNNModel(torch.nn.Module):
    def __init__(self, params):
        ...
        self.rela_embed2 = nn.Embedding(2 * self.n_rel + 1, self.attn_dim)
        ...
        self.gnn_layers2 = []
        self.gnn_layers2.append(GNNLayer(self.params, hidden_dim=self.attn_dim))
    ...
    def forward(self, triples, subgraph, mode='train', epoch=0):
        ...
        for i in range(self.n_layer-1):
            ...
            hidden = self.gnn_layers[0].forward(query, layer_input, edges, nodes, n_ent, node_edge_count)
            if mode == "train" and epoch > 3:
            ...
            layer_input = hidden
        layer_output = torch.zeros((batch_size, n_ent, self.hidden_dim), device=q_sub.device)
        ...
        for i in range(self.n_layer2):
            hidden = self.gnn_layers2[0].forward2(query, layer_input, filter_data, n_ent, edge_count.T.repeat(len(query), 1))  # filter_data
            layer_input = hidden + layer_input

        node_query = self.rela_embed2(q_rel).unsqueeze(1).expand(-1, n_ent, -1)
    ...
    def get_neighbors(self, nodes, batchsize, total_node_1hot=0, subgraph=None, filter_mask=None, layer_id=0):
        ...
        head_nodes, head_index, head_count = torch.unique(sampled_edges[:, [0, 1]], dim=0, sorted=True, return_inverse=True, return_counts=True)
        ...
        if layer_id > 0:
            ...
            last_mask = last_mask + last_mask2 # + node_mask
